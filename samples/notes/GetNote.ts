...
export class GetNote {
                ...
                if (responseObject instanceof ZOHOCRMSDK.Notes.ResponseWrapper) {
                    ...
                    notes.forEach(note => {
                        ...
                        if (parentId != null && parentId != undefined) {
                            ...
                            let keyValues: Map<string, any> = parentId.getKeyValues();
                            let keyArray = Array.from(keyValues.keys());
                            for (let keyName of keyArray) {
                                let value: any = keyValues.get(keyName);
                                if (value instanceof Map) {
                                    console.log("Record keyName: " + keyName);
                                    console.log(JSON.stringify(value, (key, value) => (value instanceof Map ? [...value] : value)));
                                }
                                else {
    ...
    static async initializeAndCall() {
        ...
        await (await new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await GetNote.getNote(BigInt("34770315001"));
    }
}
